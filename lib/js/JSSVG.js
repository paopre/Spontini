...
const jsSVGRegex = /(?:\\jsShape|\\jsOnceOverride|\\jsOnceOffset|\\jsTweak|\\jsOSOPadder) +"[^"]+" +"[^"]+" +#'\((?:(?=([^()]*))\1|\((?:(?=([^()]*))\2|\((?:(?=([^()]*))\3|\((?=([^()]*))\4\))+\))+\))+\)/gm;
...
function sendMsgToPythonHelper(data, onResponseReceived) {
  ...
  fetch(request).then(response => {
    ...
    alert("Error while connecting to JSSVGCompileHelper. \n" +
...
document.getElementById("donamelilypondlilebutton").addEventListener('click', function (ev) {
  ...
  if (!fileName.endsWith(".ly"))
    ...
    sendMsgToPythonHelper(data2, function(statusTxt0, responseTxt0) {
      var doSave = true
      console.log("RESTEXT = "+responseTxt0)
      if (responseTxt0 == "YES")
...
function getCSTStartLine(cursorLine) {
  var ret = null
...
function formatCrossStaffTable() {

  var cstStartLine = getCSTStartLine(txtEditor.getCursor().line)
  if (cstStartLine != null) {
    var currentCellNumber = getCSTCurrentCellNumber(cstStartLine, txtEditor.getCursor().line, txtEditor.getCursor().ch)
    if (currentCellNumber != null) {
      var lengthOfContentOfCellToFormatOnCSTLine = []

      for (var i = 0; i < 3; i++) {
        lengthOfContentOfCellToFormatOnCSTLine[i] = getCSTCellContentLength(cstStartLine, (i +1), currentCellNumber, true)
      }

      if ((lengthOfContentOfCellToFormatOnCSTLine[0] == null) ||
          (lengthOfContentOfCellToFormatOnCSTLine[1] == null) ||
          (lengthOfContentOfCellToFormatOnCSTLine[2] == null))
        return

      var maxContentLength = Math.max(lengthOfContentOfCellToFormatOnCSTLine[0], lengthOfContentOfCellToFormatOnCSTLine[1], lengthOfContentOfCellToFormatOnCSTLine[2])
      for (var i = 0; i < 3; i++) {
        lengthOfContentOfCellToFormatOnCSTLine[i] = getCSTCellContentLength(cstStartLine, (i +1), currentCellNumber)
        ...
        while (lengthOfContentOfCellToFormatOnCSTLine[i] != maxContentLength) {
          if (lengthOfContentOfCellToFormatOnCSTLine[i] < maxContentLength) {
            removeOrAddEndWhiteCharOfCell(cstStartLine, (i + 1), currentCellNumber, " ")
