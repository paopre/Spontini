...
function __updatePointAndClickPositions(oldText,newText) {
  ...
  nodesWithTextPos = graphicContainerCtx.document.querySelectorAll('[textpos]')
  for (var i = 0; i < nodesWithTextPos.length; i++) {
    var prevPos = Number(nodesWithTextPos[i].getAttribute('textpos'))
  ...
  if (pdfAnnotations) {
    var pageAnnotationsWithTextPos = pdfAnnotations.get(parseInt(baseCtx.pagenumtxt.value))
    if (!pageAnnotationsWithTextPos)
      return
    for (var i = 0; i < pageAnnotationsWithTextPos.length; i++)  {
      var prevPos = Number(pageAnnotationsWithTextPos[i].textPos)
      if (prevPos >= diffTxt[0]) {
        pageAnnotationsWithTextPos[i].textPos = prevPos+offset
      }
    }
  }
...
function swapMergeTableCellsUp() {
  swapMergeTableCells("UP")
}

function swapMergeTableCellsDown() {
  swapMergeTableCells("DOWN")
}
...
function initKeymapsForTxtEditor() {
  ...
  keyMaps.push({ [swapMergeTableCellsUpKey] : swapMergeTableCellsUp })

  keyMaps.push({ [swapMergeTableCellsDownKey] : swapMergeTableCellsDown })

  keyMaps.push({ [convertPitchesToEnharmonicKey] : convertPitchesToEnharmonic })
...
function adjustScroll(xOnly = false) {
  return adjustScrollIMPL(xOnly)
}
...
function swapMergeTableCells(dir) {
  swapMergeTableCellsIMPL(dir)
}

function convertPitchesToEnharmonic() {
  convertPitchesToEnharmonicIMPL()
}
...
function getDocLanguage() {
  return getDocLanguageIMPL()
}

function replaceWithVarFunc(from = null, to = null) {
  replaceWithVarFuncIMPL(from, to)
}

function replaceAllWithVarsFuncs() {
  replaceAllWithVarsFuncsIMPL()
}
