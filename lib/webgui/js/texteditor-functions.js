...
function includeExcludePages() {

  var rangeListArr = prompt("Insert selections to exclude in the form 'SELECTIONNUM' or 'SELECTIONNUM1-SELECTIONNUM2', " +
                      "separated by ';' \n" +
                      "Leave the form empty for including all the selections. \n\n" +
                      "Example: 1;3-5;9 \n")

  if (rangeListArr == null)
    return

  var prevCurs = txtEditor().getCursor()
  var scroll = txtEditor().getScrollInfo()
  var rangeList = rangeListArr.split(';')
  lines = getLyFileContent().split('\n')

  for (var q = 0; q < lines.length; q++) {
    if ((lines[q].includes("%{") || lines[q].includes("%}")) &&
        lines[q].includes(selectionIdentifier)) {
      lines[q] = lines[q].replace("%{ ", "")
      lines[q] = lines[q].replace("%} ", "")
      lines[q] = lines[q].replace("%{", "")
      lines[q] = lines[q].replace("%}", "")
    }
  ...
  for (var i = 0; i < rangeList.length; i++) {
    ...
    var rangeL = parseInt(rangeList[i].split('-')[0])
    var rangeR = NaN
    var onePage = false
    if (rangeList[i].split('-').length > 1) {
      rangeR = parseInt(rangeList[i].split('-')[1])
      if (rangeR != NaN)
        rangeR = rangeR + 1
    }
    else {
    ...
    if ((rangeL != NaN) && (rangeR != NaN) && (rangeL < rangeR)) {
      for (var q = 0; q < lines.length; q++) {
        if (lines[q].includes(selectionIdentifier) && lines[q].trim().endsWith(rangeL))
          lines[q] = lines[q].replace(selectionIdentifier, "%{ "+selectionIdentifier)
        else if (lines[q].includes(selectionIdentifier) && lines[q].trim().endsWith(rangeR))
          lines[q] = lines[q].replace(selectionIdentifier, "%} "+selectionIdentifier)
      }
    }
...
function initKeymapsForTxtEditor() {
  ...
  keyMaps.push({ [completeTableKey] : completeTableFunc })
...
function completeTableFunc(txtEditor) {
  completeTableFuncIMPL(txtEditor)
}
