...
function execTool(tool, txtEditor) {
  ...
  if (tool == 'jstweak') {
  ...
  else if (tool == 'includeexcludeselections') {
    includeExcludePages()
    executingToolInProgress = false
    exectoolbtn.style = exectoolbtndefaultstyle
    nothingElseToDo = true
    ...
    if (svgContainerCtx.document.getElementById("axes0") != null)
    {
      var axes = []
      for (var i = 0; i < 4; i++) {
        axes.push(svgContainerCtx.document.getElementById("axes"+i))
  ...
  else if (tool == 'compileselectedchunk') {
    var selection = txtEditor().getSelection()
    if (selection == '') {
      ...
      var chunkContent = selection
      contentCommonArr = getLyFileContent().split(/%__COMMONDELIMITER__/gs)
      if (contentCommonArr.length != 3) {
        alert("Error! Chunks must be placed between TWO blocks delimited by\n" +
              " two %__COMMONDELIMITER__ tags.\n\n" +
      ...
      var firstDelimiterPos = contentCommonArr[0].split(/\r\n|\r|\n/).length - 1
      var secondDelimiterPos = firstDelimiterPos+contentCommonArr[1].split(/\r\n|\r|\n/).length - 1
      var selectedTextFromLine = txtEditor().getCursor("from").line
      var selectedTextToLine = txtEditor().getCursor("to").line
      var selectedTextFromCh = txtEditor().getCursor("from").ch
      var selectedTextToCh = txtEditor().getCursor("to").ch
      var blankLinesBetween1 = selectedTextFromLine - firstDelimiterPos
      var blankLinesBetween2 = secondDelimiterPos - selectedTextToLine

      //logInfo("pos1 = " + firstDelimiterPos)
      //logInfo("pos2 = " + secondDelimiterPos)
      //logInfo("selectedTextFromLine = " + selectedTextFromLine)
      //logInfo("selectedTextToLine = " + selectedTextToLine)
      //logInfo("selectedTextFromCh = " + selectedTextFromCh)
      //logInfo("selectedTextToCh = " + selectedTextToCh)
      //logInfo("blankLinesBetween1 = " + blankLinesBetween1)
      ...
      for (var i = 0; i < selectedTextFromCh; i++)
        dummyChars1 += " "
      for (var i = 0; i < selectedTextToCh; i++)
        dummyChars2 += " "
      for (var i = 0; i < blankLinesBetween1; i++)
        dummyLines1 += "\n"
      for (var i = 0; i < blankLinesBetween2; i++)
        dummyLines2 += "\n"

      var newContent = contentCommonArr[0] + dummyLines1 + dummyChars1 +
                       chunkContent + dummyChars2 + dummyLines2 + contentCommonArr[2]
      var data = {
        cmd: 'COMPILE',
        param1: "tmp"+generateRandomString(5)+".ly",
        param2: newContent,
        param3: "null"
      }
      sendMsgToSpontiniServer(data, function(statusTxt, responseTxt) {
        if(statusTxt.startsWith("OK")) {
          ...
          alert("Error while compiling the selected chunk. \n" +
...
function addEventListenersToWidgets() {
  ...
  toolsbtn.addEventListener('click', function onOpen() {
    ...
    selectOptions += "<option value='compileselectedchunk'>Compile selected chunk</option>"
    ...
    selectOptions += "<option value='includeexcludeselections'>Include/exclude selections</option>"
