...
function getDocLanguageIMPL() {
  var ret = "nederlands"

  var langs = ["nederlands",
               "català",
               "catalan",
               "deutsch",
               "english",
  ...
  var firstLine = txtEditor().getLine(0)
  if (firstLine.includes("\\language")) {
    for (var i = 0; i < langs.length; i++)
      if (firstLine.includes(langs[i])) {
        ret = langs[i]
        break
      }
  }
...
function swapMergeTableCellsIMPL(dir) {

  var cursInsidePianoTable = false
  var cstStartLineInfo = getTableStartLineInfos(txtEditor().getCursor().line)
  var isEasyCrossStaff = cstStartLineInfo[2] == "easyCrossStaff"

  cursInsidePianoTable = cstStartLineInfo[3]

  if (cursInsidePianoTable) {

    tableRowNum = cstStartLineInfo[4]

    if ((tableRowNum != 1) && (tableRowNum !=2))
      return

    if ((dir == "UP") && (tableRowNum == 1))
      return

    if ((dir == "DOWN") && (tableRowNum == 2))
      return

    var currentCellNumber = getCSTCellNumber(txtEditor().getCursor().line,
                                             txtEditor().getCursor().ch)

    var txtFrom = getCSTCellContent(txtEditor().getCursor().line, currentCellNumber)
    var txtTo = ""

    txtFromLine = txtEditor().getCursor().line
    txtToLine = -1
    if (tableRowNum == 1)
      txtToLine = txtEditor().getCursor().line + 1
    else
      txtToLine = txtEditor().getCursor().line - 1

    txtTo = getCSTCellContent(txtToLine, currentCellNumber)

    var fragmentStartOffs = -1
    var fragmentEndOffs = -1
    var startCh = getCSTCellStartCh(txtEditor().getCursor().line, currentCellNumber)

    if ((txtEditor().getSelection().trim()) != '' && (txtEditor().listSelections().length > 0)) {
      var selection = txtEditor().listSelections()[0]
      fragmentStartOffs = Math.min((selection.head.ch - startCh), (selection.anchor.ch - startCh))
      fragmentEndOffs = Math.max((selection.head.ch - startCh - 1), (selection.anchor.ch - startCh - 1))
    }
    else {
      fragmentStartOffs = txtEditor().getCursor().ch - startCh
      fragmentEndOffs = fragmentStartOffs
    }

    if ((fragmentStartOffs != -1 ) && (fragmentEndOffs != -1)) {
      //logInfo("")
      //logInfo("[A]  "+txtFrom + "  ===>  " + txtFrom.substring(fragmentStartOffs, fragmentEndOffs+1))
      //logInfo("[B]  "+txtTo)
      var data = {
        cmd: 'MERGE_TABLE_CELLS',
        param1: txtFrom,
        param2: fragmentStartOffs,
        param3: fragmentEndOffs,
        param4: txtTo,
        param5: getDocLanguage()
      }
      sendMsgToSpontiniServer(data, function(statusTxt, responseTxt) {
        if (statusTxt == "OK") {
          startCh = getCSTCellStartCh(txtFromLine, currentCellNumber)
          disableTableFormat = true
          var newTxtFrom = responseTxt.split(templatesSepTkn)[0]
          var newTxtTo = responseTxt.split(templatesSepTkn)[1]
          txtEditor().replaceRange(newTxtFrom, {line: txtFromLine, ch: startCh},
                                               {line: txtFromLine, ch: startCh + txtTo.length})

          txtEditor().replaceRange(newTxtTo, {line: txtToLine, ch: startCh},
                                             {line: txtToLine, ch: startCh + txtFrom.length})
          disableTableFormat = false
          //logInfo("->")
          //logInfo("[A]  "+newTxtFrom)
          //logInfo("[B]  "+newTxtTo)
        }
      })
    }
...
function convertPitchesToEnharmonicIMPL() {

  var cursInsidePianoTable = false
  var currLine = txtEditor().getCursor().line
  var cstStartLineInfo = getTableStartLineInfos(currLine)
  ...
  if (cursInsidePianoTable) {
    currentCellNumber = getCSTCellNumber(currLine, txtEditor().getCursor().ch)
    textToParse = getCSTCellContent(currLine, currentCellNumber)
    startCh = getCSTCellStartCh(currLine, currentCellNumber)
  }
  else {
    textToParse = txtEditor().getLine(currLine)
  }
  ...
  var data = {
    cmd: 'CONVERT_PITCHES_TO_ENHARMONIC',
    param1: textToParse,
    param2: fragmentStartOffs,
    param3: fragmentEndOffs,
    param4: getDocLanguage()
  }
  sendMsgToSpontiniServer(data, function(statusTxt, responseTxt) {
    if ((statusTxt == "OK") && (responseTxt != textToParse)) {
      var prevCurs = txtEditor().getCursor()
      txtEditor().replaceRange(responseTxt, {line: currLine, ch: startCh},
                                            {line: currLine, ch: startCh + textToParse.length})
      txtEditor().setCursor(prevCurs)
    }
  })
