...
var showjssvg = "true"
var marker1ColorRGB   = "rgb(12.3000%, 45.6000%, 78.9000%)"
// On 2.25.x series there's not rgb anymore but rgba
var marker1ColorRGBA  = "rgba(12.3000%, 45.6000%, 78.9000%, 100.0000%)"
var marker2ColorRGB   = "rgb(98.7000%, 12.3000%, 45.6000%)"
var marker2ColorRGBA  = "rgba(98.7000%, 12.3000%, 45.6000%, 100.0000%)"
var osPadderColorRGB  = "rgb(91.2000%, 93.4000%, 95.6000%)"
var osPadderColorRGBA = "rgba(91.2000%, 93.4000%, 95.6000%, 100.0000%)"
var pdfDoc = null
var base64pdfData = null
var PDFRenderingInProgress = false
var wheelTimeoutHandler = null;
var wheelTimeout = 250 //ms
var pdfAnnotations = null
var currPdfAnnotationIdx = -1
...
function setCommonAttrsForOSOPadderLine (line, parentNode, propName) {
  ...
  line.setAttribute('onmouseover',
                    'this.setAttributeNS(null, "stroke", "'+selectGrobColor+'");'+
...
function initOSOPadder(n1, propName) {
  ...
  if (showjssvg == 'false') {
    bottomLine.setAttribute('visibility', 'hidden')
    topLine.setAttribute('visibility', 'hidden')
...
function selectOSOPaddable (grob) {
  ...
  if (panZoomOn() || isSVGReadOnly() || showjssvg == "false")
    return
...
function initShortenPair(n1, propName) {
  ...
  */
  ...
  barLeft.setAttribute('isbar', 'true')
  ...
  barRight.setAttribute('isbar', 'true')
  ...
  if (showjssvg == 'false') {
    barLeft.setAttribute('visibility', 'hidden')
    barRight.setAttribute('visibility', 'hidden')
  }
...
function initXYPlacement(n1, propName) {
  ...
  n1.setAttribute('showjssvg', showjssvg)
  var origColor = n1.getAttribute('color')
  if (origColor == null)
    origColor = "black"
  n1.setAttribute('origcolor', origColor)
...
function setCpsOnPath(path, cps) {
  ...
  path.setAttribute('stroke-width', curvesStrokeWidth)
...
function initShape(node, propName) {
  ...
  for (var q = 0; q < circles.length; q++) {
    ...
    if (showjssvg == 'false')
      circles[q].setAttribute('visibility', 'hidden')
    ...
    circles[q].setAttribute('onmouseout', 'this.setAttribute("color", "#fc560390")')
    cpsCount = cpsCount + 1
  }
  ...
  for (var q = 0; q < lines.length; q++) {
    ...
    if (showjssvg == 'false')
      lines[q].setAttribute('visibility', 'hidden')
    lineCounter++
  }
  ...
  if (paths.length > 0) {
    if (paths[0].hasAttribute('transform'))
      paths[0].removeAttribute('transform')

    if (paths[0].parentNode.hasAttribute('transform'))
      paths[0].parentNode.removeAttribute('transform')

    setCpsOnPath(paths[0], cps)

    paths[0].setAttribute('id', gid + '_path')
    paths[0].setAttribute('fill', 'none')

    if (paths.length > 1)
      if (gid.endsWith("dashed") || gid.endsWith("dotted")) {
        for (var i = 1; i < paths.length; i++)
          paths[i].setAttribute('visibility', 'hidden')
        if (gid.endsWith("dashed"))
          paths[0].setAttribute('stroke-dasharray', dashPatternForSlurs)
        else //dotted
          paths[0].setAttribute('stroke-dasharray', dotPatternForSlurs)
      }
  ...
  if (!(lilypondVersion.startsWith("2.19") ||
      lilypondVersion.startsWith("2.20") ||
      lilypondVersion.startsWith("2.21") ||
      lilypondVersion.startsWith("2.22"))) {
...
function initPositions(n1, propName) {
  ...
  for (var q = 0; q < editableNodes.length; q++) {
    ...
    editableNodes[q].setAttribute('onmouseout', 'mouseOutModifiableElm(this)')
...
function setJSSVGVisibilityForLyGeneratedObjs(val) {

  var visibility = 'visible'
  if (val == 'false')
    visibility = 'hidden'

  var bars = document.querySelectorAll('svg [isbar="true"]')
  for (var i = 0; i < bars.length; i++)
    bars[i].setAttribute('visibility', visibility)

  var cps = document.querySelectorAll('svg [id*="_cp_"]')
  for (var i = 0; i < cps.length; i++)
    cps[i].setAttribute('visibility', visibility)

  var lines = document.querySelectorAll('svg [id*="_line_"]')
  for (var i = 0; i < lines.length; i++)
    lines[i].setAttribute('visibility', visibility)

  var osopadderlines1 = document.querySelectorAll('svg [color="'+osPadderColorRGB+'"]')
  var osopadderlines2 = document.querySelectorAll('svg [paddir]')
  var osopadderlines = []
  osopadderlines.push(...osopadderlines1, ...osopadderlines2)
  for (var i = 0; i < osopadderlines.length; i++)
    osopadderlines[i].setAttribute('visibility', visibility)

  var noteColumns = document.querySelectorAll('svg [editableclass*="NoteColumn"]')
  for (var i = 0; i < noteColumns.length; i++)
    noteColumns[i].setAttribute('visibility', visibility)

  var dynamicLineSpanner = document.querySelectorAll('svg [editableclass*="DynamicLineSpanner"]')
  for (var i = 0; i < dynamicLineSpanner.length; i++)
    dynamicLineSpanner[i].setAttribute('visibility', visibility)

  var markers1 = document.querySelectorAll('svg [color="'+marker1ColorRGB+'"]')
  var markers2 = document.querySelectorAll('svg [color="'+marker2ColorRGB+'"]')
  var markers3 = document.querySelectorAll('svg [spontinimarker^="true"]')
  var markers = []
  markers.push(...markers1, ...markers2, ...markers3)
  for (var i = 0; i < markers.length; i++)
    markers[i].setAttribute('visibility', visibility)

}

function setJSSVGVisibility(val) {

  setJSSVGVisibilityForLyGeneratedObjs(val)

  var aux = 'false'
  if (showjssvg == 'false')
    aux = 'true'

  var elems = document.querySelectorAll('svg [showjssvg^="'+aux+'"]')
  ...
  for (var i = 0; i < elems.length; i++) {
    ...
    elems[i].setAttribute('showjssvg', val)

    var subElems = elems[i].querySelectorAll("*")
    for (var q = 0; q < subElems.length; q++) {

      if (isMarker(subElems[q]))
        continue

      if (subElems[q].getAttribute('color') != null)
        if (val == 'true')
          subElems[q].setAttribute('color', editableObjectsColor)
        else
          subElems[q].setAttribute('color', elems[i].getAttribute('origcolor'))
    }

    if (val == 'true')
      elems[i].setAttribute('color', editableObjectsColor)
    else
      elems[i].setAttribute('color', elems[i].getAttribute('origcolor'))
...
function mouseOverModifiableElm(elm) {
  ...
  elm.setAttribute("color", selectGrobColor)
  elm.style.cursor = "pointer"

}

function mouseOutModifiableElm(elm) {

  if (showjssvg == 'false')
    return

  elm.setAttribute("color", editableObjectsColor)
...
function createInteractiveRect(x, y, w, h, color) {
  ...
  rect.setAttribute('onmouseover',
                    'this.setAttributeNS(null, "fill", "'+selectGrobColor+'");'+
                    'this.style.cursor = "pointer"')
...
function isMarker(node) {
  ...
  return (node.nodeName == 'g' &&
          ...
          ((node.getAttributeNS(null, 'color') == marker1ColorRGB)  ||
           (node.getAttributeNS(null, 'color') == marker2ColorRGB)  ||
           (node.getAttributeNS(null, 'color') == marker1ColorRGBA) ||
           (node.getAttributeNS(null, 'color') == marker2ColorRGBA)
          ))
...
function isOsPadder(node) {
  ...
  return (node.nodeName == 'g' &&
          node.getAttributeNS(null, 'color') &&
          (
            (node.getAttributeNS(null, 'color') == osPadderColorRGB) ||
            (node.getAttributeNS(null, 'color') == osPadderColorRGBA)
          )
         )
...
function createMenuItem(x, y, w, h, color, str, id, onmouseoverFunc, onclickFunc, type) {
  ...
  moverAttr = 'document.getElementById("'+id+'_rect").setAttributeNS(null, "fill", "'+selectGrobColor+'");'+onmouseoverFunc
  if (type == 'menu') {
...
function displaySVGAndInitItsInteractiveObjs(panzoomParams, content) {

  var pdfdraw = document.getElementById("pdfdraw")
  pdfdraw.style.display = "none"
  var pdfpointandclickrect = document.getElementById("pdfpointandclickrect")
  pdfpointandclickrect.style.display = "none"
  var svgdraw = document.getElementById("svgdraw")
  svgdraw.style.display = "block"
  var svgcontainer = document.getElementById("graphiccontainer")
  ...
  // prevent a Chrome issue that renders texts with weird resizings
  // see: https://jsfiddle.net/2stekjvm/9/
  var texts = document.querySelectorAll('svg text')
  if (renderSvgTextWithGeometricPrecision)
    for (var i = 0; i < texts.length; i++) {
      texts[i].setAttributeNS(null, "text-rendering", "geometricPrecision")
    }

  setJSSVGVisibilityForLyGeneratedObjs(showjssvg)
  ...
  if (svgNode) {
    ...
    if (mustInitPanZoom) {
      ...
      graphscorepanel.style.background = graphicPanelBackground
  ...
  setTimeout(() => {
    if (panzoomParams.panzoomMode == "FIT_TO_PANEL") {
      fitScorePageToPanel()
    }
    else if (panzoomParams.panzoomMode == "SAME_DIMENSION_SAME_PAN") {
      setTimeout(() => { graphicContainerCtx.panzoomInstance.zoom(panzoomParams.prevZoom) })
      setTimeout(() => { graphicContainerCtx.panzoomInstance.pan(panzoomParams.prevPan.x, panzoomParams.prevPan.y) })
      setTimeout(() => {
        graphicContainerCtx.graphscorepanel.scrollLeft = panzoomParams.prevScrollLeft
        graphicContainerCtx.graphscorepanel.scrollTop = panzoomParams.prevScrollTop
      })
    }
  })
...
function initPanZoom(active) {
  ...
  if (panzoomInstance == null) {
    ...
    panzoomInstance = Panzoom(graphiccontainer, {
      maxScale: 10,
    })

    graphiccontainer.addEventListener('panzoomchange', (event) => {
      ...
      if (document.getElementById("axes0") != null) {
        for (var i = 0; i < 4; i++) {
          document.getElementById("axes"+i).setAttributeNS(null, "stroke-width", axesThickness/panzoomInstance.getScale())
        }
      }
    })
  }
...
function activatePanZoom() {
  if (panzoomInstance != null) {
    ...
    var pdf = document.getElementById("pdfdraw")
    pdf.addEventListener('wheel', zoomWithWheel)
    pdf.addEventListener('dblclick', zoomWithDblClick)
    var pdfpointandclickrect = document.getElementById("pdfpointandclickrect")
    pdfpointandclickrect.addEventListener('wheel', zoomWithWheel)
    pdfpointandclickrect.addEventListener('dblclick', zoomWithDblClick)
...
function scaleCanvasAndRenderPDFAfterZooming() {

  clearTimeout(wheelTimeoutHandler);
  wheelTimeoutHandler = setTimeout(function() {
    var pdfdraw = document.getElementById("pdfdraw")
    pdfdraw.style.transform = "scale("+1/panzoomInstance.getScale()+")"
    if (baseCtx.pdfDoc) {
      var panzoomParams = { panzoomMode: "SAME_DIMENSION_SAME_PAN", prevPan: null, prevZoom: null,
                            prevScrollLeft: graphscorepanel.scrollLeft, prevScrollTop: graphscorepanel.scrollTop}
      renderPDFPage(parseInt(baseCtx.pagenumtxt.value), panzoomParams);
    }
  }, wheelTimeout)
}
...
function zoomWithWheel(event) {
  ...
  if (panZoomOn()) {
    ...
    if (baseCtx.currentMode == "pdf") {
      scaleCanvasAndRenderPDFAfterZooming()
    }
    //var pdfpointandclickrect = document.getElementById("pdfpointandclickrect")
    //pdfpointandclickrect.style.transform = "scale("+panzoomInstance.getScale()+")"
...
function setSVGWidth(val) {
  graphicContainerCtx.document.querySelector("svg").setAttribute("width", val)
}

function svgWidth() {
  return graphicContainerCtx.document.querySelector("svg").getAttribute("width")
}

function setSVGHeight(val) {
  graphicContainerCtx.document.querySelector("svg").setAttribute("height", val)
}

function svgHeight() {
  return graphicContainerCtx.document.querySelector("svg").getAttribute("height")
}

function svgViewBox() {
  return graphicContainerCtx.document.querySelector("svg").getAttribute("viewBox")
}

function setPDFWidth(val) {
  baseCtx.pdfWidth_ = val
}

function pdfWidth() {
  return baseCtx.pdfWidth_
}

function setPDFHeight(val) {
  baseCtx.pdfHeight_ = val
}

function pdfHeight() {
  return baseCtx.pdfHeight_
}

function currZoom() {
  return graphicContainerCtx.currZoom_
}

function setCurrZoom(val) {
  graphicContainerCtx.currZoom_ = val
}

function fitScorePageToPanel() {
  ...
  var H = baseCtx.currentMode == "svg" ? svgHeight() : pdfHeight()
  var W = baseCtx.currentMode == "svg" ? svgWidth() : pdfWidth()

  zoomFactor = Math.min((panelH - 1)/H,  (panelW - 1)/W)
  if (zoomFactor < 0)
    zoomFactor = 0
  ...
  panX = Math.max(0, (panelW - W*zoomFactor)/(2*zoomFactor))
  panY = Math.max(0, (panelH - H*zoomFactor)/(2*zoomFactor))
...
function oneToOneZoom() {
  ...
  if (baseCtx.currentMode == "pdf")
    ...
    zoomFactor = svgOrigWidth()/svgWidth()
  var prevScale = panzoomInstance.getScale()
  panzoomInstance.zoom(zoomFactor)
  if (baseCtx.currentMode == "pdf")
    scaleCanvasAndRenderPDFAfterZooming()
...
function initGraphicPanel() {
  ...
  graphiccontainer.addEventListener('mousemove',
    function (e) {
      ...
      var pointAndClickRect = document.getElementById("pdfpointandclickrect")
      pointAndClickRect.style.border = pdfPointAndClickRectStrokeWidth +" solid "+pdfPointAndClickRectColor
  })
  ...
  pdfdraw = document.getElementById("pdfdraw")
  graphiccontainer.addEventListener('mousemove', function (e) {
    if (baseCtx.currentMode == "pdf") {
      if (!panzoomInstance)
        return
      var scaleFactor = panzoomInstance.getScale()*1.333333
      var candidateRects = []
      var moveX = e.clientX - graphiccontainer.getBoundingClientRect().x
      var moveY = e.clientY - graphiccontainer.getBoundingClientRect().y
      var pageAnnotationsWithTextPos = baseCtx.pdfAnnotations.get(parseInt(baseCtx.pagenumtxt.value))
      if (!pageAnnotationsWithTextPos)
        return
      for (var i = 0; i < pageAnnotationsWithTextPos.length; i++) {
        var currRect = pageAnnotationsWithTextPos[i].pageAnnotation.rect
        var rectX1 = currRect[0]*scaleFactor
        var rectX2 = currRect[2]*scaleFactor
        var rectY1 = (pdfHeight()/1.333333 - currRect[3])*scaleFactor
        var rectY2 = (pdfHeight()/1.333333 - currRect[1])*scaleFactor
        if ((rectX1 <= moveX) && (rectX2 >= moveX) &&
            (rectY1 <= moveY) && (rectY2 >= moveY)) {
          candidateRects.push({index: i, area: (rectX2 - rectX1)*(rectY2 - rectY1)})
        }
      }
      if (candidateRects.length > 0) {
        var minRect = candidateRects.reduce((acc, item) => item.area < (acc?.area ?? Infinity) ? item : acc);
        resizeAndShowPdfAnnotationRect(pageAnnotationsWithTextPos[minRect.index].pageAnnotation)
        currPdfAnnotationIdx = minRect.index
        //showPointAndClickPosition(pageAnnotationsWithTextPos[minRect.index].textPos)
        //console.log(pageAnnotationsWithTextPos[minRect.index].pageAnnotation.unsafeUrl)
      }
      else {
        hidePdfAnnotationRect()
        currPdfAnnotationIdx = -1
      }
    }
  })

  graphiccontainer.addEventListener('click', function (e) {
    if (currPdfAnnotationIdx != -1) {
      var pageAnnotationsWithTextPos = baseCtx.pdfAnnotations.get(parseInt(baseCtx.pagenumtxt.value))
      showPointAndClickPosition(pageAnnotationsWithTextPos[currPdfAnnotationIdx].textPos)
    }
  })
...
function hidePdfAnnotationRect() {
  var pointAndClickRect = document.getElementById("pdfpointandclickrect")
  pointAndClickRect.style.display = 'none'
}

function resizeAndShowPdfAnnotationRect(pageAnnotation) {

  var scaleFactor = 1.333333
  var currRect = pageAnnotation.rect
  var rectX1 = currRect[0]*scaleFactor
  var rectX2 = currRect[2]*scaleFactor
  var rectY1 = (pdfHeight()/1.333333 - currRect[3])*scaleFactor
  var rectY2 = (pdfHeight()/1.333333 - currRect[1])*scaleFactor
  var pointAndClickRect = document.getElementById("pdfpointandclickrect")
  pointAndClickRect.style.left = rectX1
  pointAndClickRect.style.top = rectY1
  pointAndClickRect.style.width = rectX2 - rectX1
  pointAndClickRect.style.height = rectY2 - rectY1
  pointAndClickRect.style.display = 'block'

}

/*
function drawLineOnPDF(x1, y1, x2, y2, color) {
  var canvas = document.getElementById('pdfdraw')
  var ctx = canvas.getContext('2d');
  ctx.beginPath();
  ctx.strokeStyle = color;
  ctx.lineWidth = 1;
  var scaleFactor = panzoomInstance.getScale()*1.333333
  ctx.moveTo(x1*scaleFactor, (pdfHeight()/1.333333 - y1)*scaleFactor);
  ctx.lineTo(x2*scaleFactor, (pdfHeight()/1.333333 - y2)*scaleFactor);
  ctx.stroke();
  ctx.closePath();
}

function drawRectOnPDF(rect, color) {
  drawLineOnPDF(rect[0], rect[1], rect[2], rect[1], color)
  drawLineOnPDF(rect[0], rect[3], rect[2], rect[3], color)
  drawLineOnPDF(rect[0], rect[1], rect[0], rect[3], color)
  drawLineOnPDF(rect[2], rect[1], rect[2], rect[3], color)
}
*/

function getTextPositionForPageAnnotation(pageAnnotation) {
  var ret = -1
  var url = pageAnnotation.unsafeUrl
  if (url.includes('.ly')) {
    rowColTkns = url.split('.ly:')[1].split(':')
    row = Number(rowColTkns[0])
    col = Number(rowColTkns[1])
    txtRows = getLyFileContent().split('\n')
    pos = 0
...
function renderPDFPage(pageNum, panzoomParams) {

  var canvas = document.getElementById('pdfdraw')
  var ctx = canvas.getContext('2d');
  baseCtx.pdfDoc.getPage(pageNum).then(function(page) {
    var scale = 1
    if (panzoomParams != null) {
      if (panzoomParams.panzoomMode == "SAME_DIMENSION_SAME_PAN")
        scale = panzoomInstance.getScale()
      else if (panzoomInstance.panzoomMode == "FIT_TO_PANEL")
        scale = 1
    }
    //1.333333 ---> convert pts to px
    var viewport = page.getViewport({scale: scale*1.333333})
    ...
    if (!PDFRenderingInProgress) {
      ...
      renderTask.promise.then(function() {
        ...
        page.getAnnotations().then(function (pageAnnotations) {
          if(!baseCtx.pdfAnnotations.get(pageNum)) {
            pageAnnotationsWithTextPos = []
            for (var i = 0; i < pageAnnotations.length; i++) {
              pageAnnotationsWithTextPos.push({pageAnnotation: pageAnnotations[i],
                                               textPos: getTextPositionForPageAnnotation(pageAnnotations[i])})
            }
            baseCtx.pdfAnnotations.set(pageNum, pageAnnotationsWithTextPos)
          }
        })
...
function loadPDFData(base64pdfData_) {
  ...
  return base64ToUint8Array(base64pdfData_);

}

function displayPDFAndInitItsInteractiveObjs(pageNum, panzoomParams, base64pdfData_ = null) {

  var pdfdraw = document.getElementById("pdfdraw")
  pdfdraw.style.display = "block"
  var svgdraw = document.getElementById("svgdraw")
  svgdraw.style.display = "none"
  var graphiccontainer = document.getElementById("graphiccontainer")
  ...
  if (base64pdfData_ != null) {
    baseCtx.pdfAnnotations = new Map()
    initPanZoom(panZoomOn())
  }

  //get the cached pdf
  if (base64pdfData_ == null)
    renderPDFPage(pageNum, panzoomParams)
  else
    pdfjsLib.getDocument(loadPDFData(base64pdfData_)).promise.then(function(pdfDoc_) {
      baseCtx.base64pdfData = base64pdfData_
      baseCtx.pdfDoc = pdfDoc_;
      renderPDFPage(pageNum, panzoomParams)
    });
