...
OSOPadder = #(define-music-function (col thickness editable id padding arg) ((color? white) (number? OSPadderThickness) (string? "false") (number? 0) scheme? symbol-list-or-music?)
  (let ((posIdx 0)
    ...
    (if (ly:music? arg)
      (cond
        ...
        ((eq? name 'UnaCordaEvent) #{ 
          ...
          -\tweak UnaCordaPedal.stencil #(make-stencil-boxer thickness padding col) 
      ...
      (cond
        ((memv 'OttavaBracket arg) #{
          ...
          \once \override Staff.OttavaBracket.layer = #-1
        ...
        ((memv 'TupletBracket arg) #{
          ...
          #(lambda (grob) 
            (let ((outprops (ly:grob-property grob 'output-attributes)))
              ...
              (set! outprops (cons (cons "pads-propid" id) outprops))
              (ly:grob-set-property! grob 'output-attributes outprops)))   
          \once \override TupletBracket.layer = #-1
