    ...
    (let ((name (ly:music-property evt 'name))
      ...
      (cond 
        ((and (eq? name 'SkipEvent)(or (eq? beamPos 'U)(eq? beamPos 'D)))
          ...
          (set! evt #{ \once \hide Rest $fakeRest #} )
          evt) 
        (else #f)))) 
  mus))
...
easyCrossStaff = #(define-music-function (parser location osopaddable music1 music2 beamPositions) 
            ((boolean? #t) ly:music? ly:music? list?)
(let
  ((counter 0) 
   (music1Voice1 '())
   (music1Voice2 '())
